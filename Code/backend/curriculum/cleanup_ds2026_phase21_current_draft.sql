...
DROP PROCEDURE IF EXISTS cleanup_ds2026_phase21_current_test;
DELIMITER $$

CREATE PROCEDURE cleanup_ds2026_phase21_current_test()
BEGIN
    ...
    DROP TEMPORARY TABLE IF EXISTS tmp_ds2026_phase21;
    CREATE TEMPORARY TABLE tmp_ds2026_phase21 (
        syllabus_id INT PRIMARY KEY
    ) ENGINE=InnoDB;

    INSERT INTO tmp_ds2026_phase21 (syllabus_id)
    ...
    FROM tmp_ds2026_phase21;

    IF v_target_count <> 45 THEN
        SIGNAL SQLSTATE '45000'
            SET MESSAGE_TEXT = 'Cleanup stopped: expected exactly 45 linked DS2026 DRAFT syllabuses.';
    END IF;

    SELECT COUNT(*) INTO v_approval_count
    FROM approval_request ar
    JOIN tmp_ds2026_phase21 t
      ON t.syllabus_id = ar.syllabus_id;

    IF v_approval_count <> 0 THEN
        SIGNAL SQLSTATE '45000'
            SET MESSAGE_TEXT = 'Cleanup stopped: DS2026 DRAFT data has approval workflow records.';
    END IF;

    START TRANSACTION;

    UPDATE class_section cs
    JOIN tmp_ds2026_phase21 t ON t.syllabus_id = cs.syllabus_id
    SET cs.syllabus_id = NULL;

    DELETE h
    FROM syllabus_import_history h
    JOIN tmp_ds2026_phase21 t ON t.syllabus_id = h.syllabus_id;

    DELETE ss
    FROM syllabus_source_snapshot ss
    JOIN tmp_ds2026_phase21 t ON t.syllabus_id = ss.syllabus_id;

    UPDATE course_program cp
    JOIN tmp_ds2026_phase21 t ON t.syllabus_id = cp.syllabus_id
    SET cp.syllabus_id = NULL
    WHERE cp.cohort_id = v_cohort_id;

    DELETE s
    FROM syllabus s
    JOIN tmp_ds2026_phase21 t ON t.syllabus_id = s.id;
...
CALL cleanup_ds2026_phase21_current_test();
DROP PROCEDURE IF EXISTS cleanup_ds2026_phase21_current_test;
