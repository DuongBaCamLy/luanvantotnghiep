...
$sqlFile = Join-Path $scriptRoot "cleanup_ds2026_current_draft_only.sql"
...
if ($LASTEXITCODE -ne 0 -or [int]($count.Trim()) -ne 35) {
    Write-Host "ERROR: expected exactly 35 linked DS2026 DRAFT rows. Nothing changed." -ForegroundColor Red
...
$backupName = "ds2026_before_clean_reimport_$timestamp.sql"
...
Write-Host ""
Write-Host "Removing ONLY the verified 35 linked DS2026 DRAFT test syllabuses..." -ForegroundColor Cyan
...
Write-Host ""
Write-Host "DS2026 DRAFT-ONLY CLEANUP COMPLETED" -ForegroundColor Green
Write-Host "CS2021 / CS2026 / IT2026 were not selected by this cleanup." -ForegroundColor Cyan
Write-Host "DS2026 CourseProgram curriculum mappings are preserved." -ForegroundColor Cyan
